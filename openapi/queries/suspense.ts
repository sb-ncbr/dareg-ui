...
export const useApiServiceGetApiV1DatasetsSuspense = <TData = Common.ApiServiceGetApiV1DatasetsDefaultResponse, TError = unknown, TQueryKey extends Array<unknown> = unknown[]>({ page, project }: {
  page?: number;
  project?: string;
} = {}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UseApiServiceGetApiV1DatasetsKeyFn({ page, project }, queryKey), queryFn: () => ApiService.getApiV1Datasets({ page, project }) as TData, ...options });
